# Rishav Chapagain, portfolio (static export)
    ...
    assets/        portrait.webp, rishav-chapagain-resume.pdf, favicon.svg/.ico,
...
Fonts load from Google Fonts (Archivo Black, Archivo) via the `<link>` tags
in `index.html`'s `<head>`. To go fully offline, download the woff2 files
into `assets/fonts/` and replace those links with local `@font-face` rules
in `styles.css`.
